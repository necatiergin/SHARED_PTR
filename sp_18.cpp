...
int main()
{
	...
	std::shared_ptr<Ovec> svecptr(new Ovec); // use_count == 1
	svecptr->reserve(10);
	...
	std::shared_ptr<Owner> owner_ptr1(svecptr, &svecptr->at(0));
	cout << "[2] svecptr.use_count() = " << svecptr.use_count() << '\n';
	cout << "[3] owner_ptr1.use_count() = " << owner_ptr1.use_count() << '\n';
	std::shared_ptr<Owner> owner_ptr2(svecptr, &svecptr->at(1));
	...
	std::shared_ptr<Member> member_ptr(svecptr, &svecptr->at(1).mx);
